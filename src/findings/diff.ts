...
import type { ArchitectureStatus, InterfaceStatus, ScanFinding, SourceEvidence } from "../state/types.js";
...
export function createScanFindings(status: ArchitectureStatus, scan: ScanResult): ScanFinding[] {
  ...
  for (const [key, scannedInterface] of scanned) {
    if (!confirmed.has(key)) {
      ...
      const sourceEvidence = evidenceFor(key, scannedInterface.evidence, evidenceByKey);
      findings.push({
...
function evidenceFor(
  ...
  return [...(primary ?? []), ...(evidenceByKey.get(key) ?? [])].filter(uniqueEvidence);
}
