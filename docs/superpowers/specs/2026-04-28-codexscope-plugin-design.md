...
- Served mode: the report is opened through the plugin's task-scoped local viewer server, and the button calls a local refresh endpoint that reruns `codex-scope refresh`.
